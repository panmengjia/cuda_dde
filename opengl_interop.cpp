...
public:
    ...
    cv_gl() {}
    cv_gl(int width, int height, std::string& window_name, cv::VideoCapture& cap, cv::Mat& frame)
    {
        m_width       = width;
        m_height      = height;
...
//        cap.open(file.c_str());
    ...
    cap.open("/home/nvidia/Desktop/pmj_ogl/130(3).avi");
...
//    }
    cap>>frame;
    cv_gl app_cv_gl(width, height, wndname, cap, frame);
    cout <<"             "<<app_cv_gl.thread_flag<<endl;
//    cv::waitKey(10000);
    pthread_t pid;
    pthread_create(&pid,NULL,thread_callback,&app_cv_gl.thread_flag);
    pthread_detach(pid);
